...
class CrudViewCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($this->formFields as $key => $value) {
            ...
            if ($i == 0) {
                $this->formBodyHtml .= '<td><a href="{{ url(\'cms\%%routeGroup%%%%crudName%%\', $item->id) }}">{{ $item->' . $field . ' }}</a></td>';
            } else {
        ...
        // For cms_form.blade.php file
        $createFile = $this->viewDirectoryPath . 'cms_form.blade.stub';
        $newCreateFile = $path . 'cms_form.blade.php';
        if (!File::copy($createFile, $newCreateFile)) {
            echo "failed to copy $createFile...\n";
        } else {
            $this->templateCreateVars($newCreateFile);
