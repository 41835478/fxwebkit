...
class CrudCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $viewPath = ($this->option('view-path')) ? $this->option('view-path') : 'cms::forms';
...
//            $this->call('crud:lang', ['name' => $viewName, '--fields' => $fields, '--locales' => $locales]);
        ...
        $routeFile = base_path('modules/Cms/Http/formsRoutes.php');
        if (file_exists($routeFile) && (strtolower($this->option('route')) === 'yes')) {
            ...
            if (\App::VERSION() >= '5.2') {
                ...
                $routText = "Route::group(['middleware' => ['authenticate.admin'],'prefix' => 'cms', 'namespace' => ''], function() {" . "\n" . implode("\n", $this->addRoutes()) . "});";

                $routText .= "\nRoute::get('" . $this->routeName . "/form', [
    'as' => '" . $this->routeName . ".form', 'uses' => '" . $this->controller . "@cms_create'
]);";
                $routText .= "\nRoute::post('" . $this->routeName . "/form', [
    'uses' => '" . $this->controller . "@cms_store'
]);";
                $isAdded = File::append($routeFile, $routText);
            }
