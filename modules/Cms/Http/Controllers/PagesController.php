    ...
    private function getPageMoudulesName($page_id, $article_html = '') {
      ...
      (all_pages=2 and not '$page_id' in (select pages_id from cms_pages_contents_pages where pages_contents_id=first_table.id and pages_id=$page_id) )
        ...
        foreach ($page_modules as $page_module) { 
            ...
            $module_html = '<div id="'.$page_module->type.'" value="'.$page_module->module_id.'" content_id="' . $page_module->id . '"  float="'.$page_module->float.'"  all_pages="'.$page_module->all_pages.'" selected_pages="'.$page_id.'" class="module_list_button reorderable" onclick="show_module_config_form($(this));" onblur="hide_module_config_form($(this));" draggable="true" style="' . $float . $display . '">';
