...
@section('content')
        ...
        @foreach($modules_list as $key=>$module)
            <div id="{{ $key }}" content_id="0"
                ...
                @else
                    ...
                    class="dragable module_list_button" float="0" all_pages='1' selected_pages="{{ $page_id }}"
                    >{{ $module['name'] }}
                @endif
    ...
    <div class="form_row">
        <div class="radio_select_all_div">
            ...
            <label value="1"> {{ trans('cms::cms.left') }}</label>
            <label value="0">{{ trans('cms::cms.no_float') }}</label>
            <label value="2">{{ trans('cms::cms.right') }}</label>
        </div>
    </div>
    ...
    <div class="form_row">
        <div class="radio_select_all_div">
            ...
            <label value="0"> {{ trans('cms::cms.all_pages') }} </label>
            <label value="1">{{ trans('cms::cms.selected_pages') }}</label>
            <label value="2">{{ trans('cms::cms.except_pages') }} </label>
        </div>
