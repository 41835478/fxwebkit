...
<div class="container">
    ...
    <div class="table">
        <table class="table table-bordered table-striped table-hover">
            <thead>
                <tr>
                    <th>S.No</th><th>{{ trans('cms::cms.fullname') }}</th><th>{{ trans('cms::cms.mobile') }}</th><th>{{ trans('cms::cms.email') }}</th><th>Actions</th>
                </tr>
            </thead>
            <tbody>
            {{-- */$x=0;/* --}}
            @foreach($cms_forms_referringpartner as $item)
                ...
                <tr>
                    <td>{{ $x }}</td>
                    <td>{{ $item->fullname }}</td><td>{{ $item->mobile }}</td><td>{{ $item->email }}</td>
                    <td>
                        <a href="{{ url('cms\cms_forms_referringpartner', $item->id) }}" class="icon_button blue_icon fa fa-file-text tooltip_number" data-original-title={{trans('cms::cms.details')}}></a>
