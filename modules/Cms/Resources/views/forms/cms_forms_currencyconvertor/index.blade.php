...
<div class="container">
    ...
    <div class="table">
        <table class="table table-bordered table-striped table-hover">
            <thead>
                <tr>
                    <th>S.No</th><th>{{ trans('cms::cms.from') }}</th><th>{{ trans('cms::cms.to') }}</th><th>{{ trans('cms::cms.amount') }}</th><th>Actions</th>
                </tr>
            </thead>
            <tbody>
            {{-- */$x=0;/* --}}
            @foreach($cms_forms_currencyconvertor as $item)
                ...
                <tr>
                    <td>{{ $x }}</td>
                    <td>{{ $item->from }}</td><td>{{ $item->to }}</td><td>{{ $item->amount }}</td>
                    <td>
                        <a href="{{ url('cms\cms_forms_currencyconvertor', $item->id) }}" class="icon_button blue_icon fa fa-file-text tooltip_number" data-original-title={{trans('cms::cms.details')}}></a>
