    ...
    <div class="table">
        <table class="table table-bordered table-striped table-hover">
            ...
            @foreach($cms_forms_payment as $item)
                ...
                <tr>
                    <td>{{ $x }}</td>
                    <td>{{ $item->OWNER_NAME }}</td><td>{{ $item->ORDERID }}</td><td>{{ $item->EMAIL }}</td>
                    <td>
                        <a href="{{ url('cms\cms_forms_payment', $item->id) }}" class="icon_button blue_icon fa fa-file-text tooltip_number" data-original-title={{trans('cms::cms.details')}}></a>
